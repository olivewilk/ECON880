...
@everywhere @with_kw struct Primitives
    ...
    nA::Int64 = 1501 #number of asset grid points
    A_grid::SharedArray{Float64,1} = collect(range(0.0, length = nA, stop = a_max)) #grid for assets
    ...
    η::SharedArray{Float64,1} = map(x->parse(Float64,x), readlines("PS5/ef.txt")) # deterministic age-efficiency profile
    ...
    π::SharedArray{Float64,2} = [π_hh 1-π_hh; 1-π_ll π_ll] #transition matrix
    π_0::SharedArray{Float64, 1} = [0.2037, 0.7963]  # Erodic distribution of Π
...
@everywhere @with_kw mutable struct Results
    val_function::SharedArray{Float64, 3} #firm value function W
    val_w::SharedArray{Float64, 3} #value function for workers over assets and productivity
    val_r::SharedArray{Float64, 3} #value function for retired just over assets 
    pol_func::SharedArray{Float64, 3} #policy function (choice of assets/savings function)
    e::SharedArray{Float64, 2} # productivity levels
    mu_norm::SharedArray{Float64, 1} #relative size of each cohort
    F::SharedArray{Float64, 3} #Steady state distribution over age, productivity, and assets
    K::Float64 #capital supply
    L::Float64 #labor supply
    labor::SharedArray{Float64, 3} #labor supply over assets and productivity
    w::Float64 #wage
    r::Float64 #interest rate
    b::Float64 #pension
    θ::Float64 #proportional labor income tax
    γ::Float64 #weight on consumption
    z::SharedArray{Float64,1} #productivity states
end

#function for initializing model primitives and results
@everywhere function Initialize(θ::Float64, γ::Float64, z::Array{Float64,1})
    prim = Primitives() #initialize primtiives
    val_function = SharedArray{Float64}(zeros(prim.N,prim.nA, 2)) #initial value function guess
    val_w = SharedArray{Float64}(zeros(prim.N,prim.nA, 2)) #initial value function guess
    val_r = SharedArray{Float64}(zeros(prim.N,prim.nA, 2)) #initial value function guess
    pol_func = SharedArray{Float64}(zeros(prim.N,prim.nA, 2)) #initial policy function guess
    #e = prim.η.*prim.z'  # productivity levels
    e = SharedArray{Float64}(zeros(prim.N, 2))
    mu_norm = SharedArray{Float64}(zeros(prim.N)) #normalized relative size of each cohort
    F = SharedArray{Float64}(zeros(prim.N,prim.nA,2))
    K = 0 #initial capital supply guess
    L = 0 #initial labor supply guess
    labor = SharedArray{Float64}(zeros(prim.N, prim.nA, 2)) #initial labor supply guess
    w = 1.05 
    r = 0.05 
    b = 0.2 
...
@everywhere function Bellman(prim, res, θ, z, γ)
    @unpack N, n, A_grid, π, β = prim
    @unpack val_function, val_w, val_r, pol_func, e = res
    res.e = prim.η.*z'
    val_r_next = SharedArray{Float64}(zeros(N, prim.nA, 2)) .- Inf
    val_w_next = SharedArray{Float64}(zeros(N, prim.nA, 2)) .- Inf
    val_w = SharedArray{Float64}(zeros(N, prim.nA, 2)) .- Inf
    val_r = SharedArray{Float64}(zeros(N, prim.nA, 2)) .- Inf
    @sync @distributed for j in 1:2 
        for age in N:-1:1 
            for i in 1:prim.nA
                a = A_grid[i]
                if age==66
...
@everywhere function Tstar(prim::Primitives,res::Results; tol::Float64 = 1e-10, max_iter::Int64 = 1000)
...
function model_solve(prim, res; θ::Float64, z::Array{Float64,1}, γ::Float64,eps::Float64, λ::Float64, K::Float64, L::Float64) 
    ...
    while iter<max_iter 
        ...
        @everywhere Bellman(prim, res, θ, z, γ) #solve the HH problem 
        @everywhere Tstar(prim, res)
