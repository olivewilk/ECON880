...
@with_kw struct Primitives
    ...
    #c_f::Float64 = 15 #fixed cost of running a business
...
function firm_labor_demand(prim, resvectors, resfloats, s, c_f)
    @unpack p = resfloats 
    @unpack θ = prim 
    labordemand = (θ*p*s)^((1/(1-θ))) 
end 

function profit(prim,  resvectors, resfloats, s, i, c_f)
    @unpack p = resfloats 
    @unpack N = resvectors
    @unpack θ = prim  
    n=N[i]
...
function Bellman(prim,  resvectors, resfloats, c_f)
    @unpack p = resfloats 
    @unpack θ = prim 
    ...
    for i in 1:prim.ns
        s = prim.s_grid[i]
        resvectors.N[i] = firm_labor_demand(prim,  resvectors, resfloats, s, c_f)
        resvectors.π[i] = profit(prim,  resvectors, resfloats, s, i, c_f)

        W_exit = resvectors.π[i] # value of exiting is just profit 
        #println(" exit value:",W_exit)
        W_stay = resvectors.π[i] 
        for j = 1:prim.ns 
            W_stay += prim.β*sum(prim.F[i,j].*resvectors.W[j]) #value of staying is profit + continuation value 
        end 
        #W_stay = resvectors.π[i] + prim.β*sum(prim.F[i,:].*W_next) #value of staying is profit + continuation value 
        #println(" stay value:",W_stay)
        if W_stay > W_exit
            resvectors.pol_func[i] = 0
            W_next[i] = W_stay
        else 
            resvectors.pol_func[i] = 1
            W_next[i] = W_exit
        end 
...
function Bellman_shocks(prim,  resvectors, resfloats, α, c_f)
    @unpack p = resfloats 
    @unpack θ, eulergamma = prim 
...
function W_iterate(prim,  resvectors, resfloats, α, c_f)
    @unpack tol, err = prim
    n = 0 #counter
    while err>tol #begin iteration
        #println("price is: ", resfloats.p)
        if α>0 
            W_next = Bellman_shocks(prim,  resvectors, resfloats, α, c_f) 
        else 
            W_next = Bellman(prim,  resvectors, resfloats, c_f) 
        end
...
function Price_solve(prim, resvectors, resfloats, α, c_f)
    ...
    while abs(entry) > tol_p && n < 10000
        ...
        W_iterate(prim, resvectors, resfloats, α, c_f) #solve incumbent firm problem 
    ...
    return resvectors.W, resvectors.pol_func
end 
