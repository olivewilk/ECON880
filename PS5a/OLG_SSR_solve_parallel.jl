...
using Distributed
@everywhere using Parameters, Plots, LinearAlgebra, Optim, Interpolations, SharedArrays #import the libraries we want



###### First part of the question ######

include("OLG_SSR_parallel.jl") #import the functions that solve our growth model
@everywhere prim, res= Initialize(0.11, 0.42, [3.0,0.5]) #initialize primitive and results structs 
println("Solving the model with θ = 0.11, z=[3.0,0.5], γ=0.42")
@everywhere @elapsed @time Bellman(prim, res, 0.11,[3.0,0.5], 0.42)
...
#=
#### Other Parts ####
@everywhere using DataFrames, CSV #import the libraries we want
include("OLG_SSR_parallel.jl") #import the functions that solve our growth model
...
@everywhere prim, res= Initialize(0.0, 0.42, [3.0,0.5]) #initialize primitive and results structs
...
@everywhere prim, res= Initialize(0.11, 0.42, [0.5,0.5]) #initialize primitive and results structs
...
@everywhere prim, res= Initialize(0.0, 0.42, [0.5,0.5]) #initialize primitive and results structs
